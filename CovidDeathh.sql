...
SELECT location, date, total_cases,total_deaths, (total_deaths/total_cases) * 100 as DeathPercentage
FROM portfolio_project.coviddeaths
WHERE location like "%Africa%";  -- showing the likelihood of death if covid was contracted in that location, at that time (Africa)

-- exploring the total cases vs popluation to show the percentage of population that got covid - Using a continent as case study
...
SELECT location, Population, date, MAX(total_cases) as HighestInfectedCount, MAX((total_cases/Population)) * 100 as InfectedPopPercentage
FROM portfolio_project.coviddeaths
where continent is not null
...
SELECT location, sum(cast(total_deaths as UNSIGNED)) as TotalDeathsCount -- changing the datatype of the column = totaldeaths
FROM portfolio_project.coviddeaths
where continent is not null
and location not in ("World", "European Union", "International")
Group by location
Order By TotalDeathsCount desc;
SELECT * FROM portfolio_project.totaldeathscount;

-- 3. countries with highest infection rate compared to population
Create view InfectedPopPercentage as
SELECT location, Population, MAX(total_cases) as HighestInfectedCount, MAX((total_cases/Population)) * 100 as InfectedPopPercentage
FROM portfolio_project.coviddeaths
where continent is not null
Group by location, Population
Order By InfectedPopPercentage desc; -- i want to see the highest infectedpoppercentage
SELECT * FROM portfolio_project.infectedpoppercentage;

-- 4. overall across the globe - Global Numbers
Create view GlobalDeathPercentages as
SELECT sum(new_cases) as total_cases, sum(cast(new_deaths as unsigned)) as total_deaths, sum(cast(new_deaths as unsigned))/sum(new_cases) * 100 as GlobalDeathPercentages
FROM portfolio_project.coviddeaths
where continent is not null
order by 1, 2;
SELECT * FROM portfolio_project.globaldeathpercentages;